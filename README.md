...
## Сдаём задачу спринта 12 на ревью (ветка spring_12)
...
## Сдаём задачу спринта 13 на ревью (ветка sprint_13)

Вы завершили 13-й спринт и работу над проектом Image Feed. Пришло время для ревью!
Вам нужно сдать проект, в котором:
- успешно протестированы краевые случаи с помощью Charles Proxy
- проведён рефакторинг вьюконтроллера WebViewViewController
- ProfileViewController и ImageListViewController покрыты Unit-тестами
...
- В WebViewViewController оставлена только его основная ответственность — работа с UI.
- Для всех остальных ответственностей созданы новые сущности:
    - логика работы вынесена в презентер;
    - сделан вспомогательный класс-хелпер;
    - создана структура, которая будет хранить пути для работы с API.
2. WebViewController покрыт Unit-тестами:  
- Напиcан тест testPresenterCallsLoadRequest, чтобы проверить, вызывает ли презентер после вызова presenter.viewDidLoad() метод loadRequest вьюконтроллера.
- Написан тест testProgressHiddenWhenOne — если значение прогресса равно единице, то метод возвращает true.
- Написан тест testCodeFromURL, который проверяет, что AuthHelper корректно распознаёт код из ссылки.
3. ProfileViewController и ImageListViewController покрыты Unit-тестами.
4. Написан UI тест, проверяющий авторизацию в приложение:
- этот сценарий предполагается описать в функции testAuth():
```
...
```
5. Написан UI тест, проверяющий работу с лентой:
- этот сценарий предполагается описать в функции testFeed() по такому принципу:
```
...
```
6. Написан UI тест, проверяющий работу с профилем:
- этот сценарий предполагается описать в функции testProfile()по такому принципу:
```
...
```
7. Приложение должно поддерживать устройства iPhone с iOS 13 или выше, предусмотрен только портретный режим.